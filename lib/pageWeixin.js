...
exports.index = function (req, res, next) {
    ...
    var wx = wechat(wxconfig, function (req, res, next) {
        ...
        if (message.MsgType == 'text') {
            ...
            //记录位置信息
            baidumap.geoconv(weizhi.Longitude,weizhi.Latitude, function (err, v1) {

                    if(!err&&v1) {

                        var v = tool.parseJSON(v1);


                        if(v.status==0) {

                            weizhi.baidux =v.result[0].x;
                            weizhi.baiduY = v.result[0].y;

                            fs.writeFile('../logs/Position/' + FromUserName + '.txt', JSON.stringify(weizhi), function (e, v) {

                                //  console.log("位置信息存取", e, v);

                            });
                        }
                        else
                        {
                            console.log("百度地图获取坐标失败", e,v1);
                        }
                    }

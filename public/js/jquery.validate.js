...
(function(){
	...
	jQuery.extend(jQuery.validator.messages, {
		...
		email: "请输入正确格式的电子邮件",
		url: "请输入格式正确的网址",
		date: "格式示例：2012-03-19",
		number: "请输入数字",
		digits: "请输入整数",
		equalTo: "请再次输入相同的值",
		maxlength: jQuery.validator.format("最多输入 {0} 个字"),
		minlength: jQuery.validator.format("至少要输入 {0} 个字"),
		rangelength: jQuery.validator.format("长度要介于 {0} 和 {1} 之间"),
		range: jQuery.validator.format("请输入一个介于 {0} 和 {1} 之间的数字"),
		max: jQuery.validator.format("请输入一个最大为 {0} 的数字"),
		min: jQuery.validator.format("请输入一个最小为 {0} 的数字")
	});
	...
	$.validator.addMethod("maxlength2", function(value, element, param){
		return this.optional(element) || getByteLength($.trim(value), element) <= param;
	}, $.validator.format("最多输入{0}个字"));
	
	$.validator.addMethod("minlength2", function(value, element, param){
		return this.optional(element) || getByteLength($.trim(value), element) >= param;
	}, $.validator.format("至少要输入{0}个字"));
	
	$.validator.addMethod("rangelength2", function(value, element, param){
		var length = getByteLength($.trim(value), element);
		return this.optional(element) || (length >= param[0] && length <= param[1]);
	}, $.validator.format("长度要介于 {0} 和 {1} 之间"));
	...
	*/
	jQuery.validator.addMethod("skip_or_fill_minimum", function(value, element, options) {
		...
		return valid;
	}, jQuery.format("可以都不填写，或者至少填写{0}项"));
	...
	*/
	...
	}, jQuery.format("支持格式png、jpg、jpeg、gif."));
	
	jQuery.validator.addMethod("alphanumeric", function(value, element, param) {
		return this.optional(element) || /^[\w-]+$/i.test(value);
	}, jQuery.format("只能输入字母、数字、-或_"));
	
    $.validator.addMethod("positiveDigits", function(value, element, param){
